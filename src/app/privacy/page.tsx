...
const SECTIONS: { heading: string; body: string }[] = [
  ...
  {
    heading: "Analysis chat",
    body: "When you use the analysis chat to ask about a result, the original message and that analysis are sent to the AI provider (such as OpenCode Zen) to generate the answer. Voice reading of translated summaries uses your device's built-in speech engine and never sends text anywhere.",
  },
  {
    heading: "Share links",
    body: "Share links are created by encrypting the analysis with a server-side secret (AES-256-GCM), so the raw result can't be read from the URL alone; the shared page is generated by decrypting it on the server. The raw input is only included when you choose to include it, and when you mark a share as sensitive the input is never sent to the page. Links do not expire, and are not stored on our servers. Anyone with a working link can open the share, so treat it like a document link.",
  },
  {
